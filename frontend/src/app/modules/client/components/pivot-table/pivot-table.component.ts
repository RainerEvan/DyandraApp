...
export class PivotTableComponent implements OnInit {
  ...
  constructor(private reportService:ReportService, private clientService:ClientService, private dialogService:DialogService, private route:ActivatedRoute) {
      this.chartType = [
  ...
  generateReport(reportId:any){
      this.clientService.generateReport(reportId).subscribe({
          next:(response:any)=>{
  ...
  saveReport(title:string){
      ...
      this.clientService.saveReport(this.report.id, reportRequest).subscribe({
          next:(response:any)=>{
